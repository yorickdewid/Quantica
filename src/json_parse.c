...
static const long flag_next				= 1 << 0;
static const long flag_reproc			= 1 << 1;
static const long flag_need_comma		= 1 << 2;
static const long flag_seek_value		= 1 << 3;
static const long flag_escaped			= 1 << 4;
static const long flag_string			= 1 << 5;
static const long flag_need_colon		= 1 << 6;
static const long flag_done				= 1 << 7;
static const long flag_num_negative		= 1 << 8;
static const long flag_num_zero			= 1 << 9;
static const long flag_num_e			= 1 << 10;
static const long flag_num_e_got_sign	= 1 << 11;
static const long flag_num_e_negative	= 1 << 12;
...
static void *default_alloc(size_t size, int zero, void *user_data) {
	(void)user_data;
	return zero ? zcalloc(1, size) : zmalloc(size);
}
			...
			if (flags & flag_done) {
				...
				switch (b) {
					whitespace:
						continue;
					default:
						sprintf(error, "%u:%u: Trailing garbage: `%c`", state.cur_line, state.cur_col, b);
						goto e_failed;
				};
			}
					...
					default:
						if (flags & flag_need_comma) {
							if (b == ',') {
								...
								sprintf(error, "%u:%u: Expected , before %c", state.cur_line, state.cur_col, b);
								goto e_failed;
							}
						}
