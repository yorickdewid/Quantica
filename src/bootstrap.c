...
void bootstrap(struct engine *e) {
	...
	md.exec = TRUE;
	md.type = MD_TYPE_RAW;
	if (engine_insert_meta_data(e, &key, &md, data0, strlen(data0)) < 0)
	...
	md.syslock = LOCK;
	md.type = MD_TYPE_RAW;
	if (engine_insert_meta_data(e, &key, &md, data1, strlen(data1)) < 0)
		lprint("[erro] bootstrap: Insert failed\n");
	...
	md.exec = TRUE;
	md.type = MD_TYPE_RAW;
	if (engine_insert_meta_data(e, &key, &md, data2, strlen(data2)) < 0)
		lprint("[erro] bootstrap: Insert failed\n");
	...
	md.syslock = LOCK;
	md.type = MD_TYPE_RAW;
	if (engine_insert_meta_data(e, &key, &md, data3, strlen(data3)) < 0)
		lprint("[erro] bootstrap: Insert failed\n");
}
