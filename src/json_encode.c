...
static const json_serialize_opts default_opts = {json_serialize_mode_packed, 0, 0};
...
json_value *json_array_new(size_t length) {
	...
	if (!(value->u.array.values = (json_value **)zmalloc(length * sizeof(json_value *)))) {
		zfree(value);
		return NULL;
	}
...
json_value *json_array_push (json_value *array, json_value *value) {
	...
	if (((json_builder_value *)array)->additional_length_allocated > 0) {
		...
		json_value **values_new = (json_value **)zrealloc(array->u.array.values, sizeof(json_value *) * (array->u.array.length + 1));
...
json_value *json_object_push_nocopy(json_value *object, unsigned int name_length, json_char *name, json_value *value) {
	...
	if (((json_builder_value *)object)->additional_length_allocated > 0) {
		...
		json_object_entry *values_new = (json_object_entry *)zrealloc(object->u.object.values, sizeof(*object->u.object.values) * (object->u.object.length + 1));
...
json_value *json_object_merge(json_value *object_a, json_value *object_b){
	...
	if (object_b->u.object.length <= ((json_builder_value *) object_a)->additional_length_allocated) {
		((json_builder_value *) object_a)->additional_length_allocated -= object_b->u.object.length;
	} else {
	...
	for (i=0; i<object_b->u.object.length; ++i) {
		json_object_entry * entry = &object_a->u.object.values[object_a->u.object.length + i];

		*entry = object_b->u.object.values[i];
		entry->value->parent = object_a;
	}

	object_a->u.object.length += object_b->u.object.length;

	zfree(object_b->u.object.values);
	zfree(object_b);
	...
	while (value) {
		...
		switch (value->type) {
			...
			case json_double:
				...
				break;
			case json_boolean:
				total += value->u.boolean ? 4:  /* `true` */ 5;  /* `false` */
				break;
			case json_null:
				total += 4;  /* `null` */
				break;
	...
	while (value) {
		switch (value->type) {
			...
			case json_object:
				...
				if (((json_builder_value *) value)->is_builder_value) {
					zfree(value->u.object.values [value->u.object.length].name);
				}

				value = value->u.object.values [value->u.object.length].value;
				continue;
			case json_string:
				zfree(value->u.string.ptr);
				break;
			default:
				break;
		};
