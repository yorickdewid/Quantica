...
LDFLAGS= -lm
SOURCES=$(SRCDIR)/common.c $(SRCDIR)/time.c $(SRCDIR)/log.c $(SRCDIR)/zmalloc.c $(SRCDIR)/strlcpy.c $(SRCDIR)/strlcat.c $(SRCDIR)/itoa.c $(SRCDIR)/strdup.c $(SRCDIR)/quid.c $(SRCDIR)/sha1.c \
        $(SRCDIR)/aes.c $(SRCDIR)/base64.c $(SRCDIR)/crc32.c $(SRCDIR)/md5.c $(SRCDIR)/sha256.c $(SRCDIR)/engine.c $(SRCDIR)/core.c $(SRCDIR)/hashtable.c \
        $(SRCDIR)/bootstrap.c $(SRCDIR)/webapi.c $(TESTDIR)/benchmark-engine.c $(TESTDIR)/test-quid.c $(TESTDIR)/test-aes.c \
        $(TESTDIR)/test-base64.c $(TESTDIR)/test-crc32.c $(TESTDIR)/test-sha1.c $(TESTDIR)/test-md5.c $(TESTDIR)/benchmark-quid.c \
